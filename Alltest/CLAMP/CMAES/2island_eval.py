...
def save_lasts(dirs):
    ...
    for idx, directory in enumerate(directories):
        ...
        if idx == 0:
            ...
            np.savetxt('last.csv', [params], delimiter=',', header = ','.join(headers), newline='\n')
        else:
            with open('last.csv','ab') as f:
                np.savetxt(f, [params], delimiter=',',  newline='\n')
...
if __name__ == '__main__':
    ...
    for instance_index, directory in enumerate(directories):
        ...
        best_results.append(sorted_result.minimums[-1])

    save_best_costs(best_results)

...
class OptimizationSettings(MetaOptimizationSettings):
    ...
    def __init__(self, xml_file, directory):
        ...
        try:
            self.xml = ET.parse('hh_pas_settings.xml')
        except IOError:
    ...
    def get_optimization_settings(self):
        ...
        for child in root:
            ...
            if child.tag == "num_islands":
                self.number_of_islands = int(child.text)
            if child.tag == "feats":
...
class GeneralPlotter(object):
    ...
    def create_generation_plot(self, statistics, title=''):
        ...
        plt.savefig('island.png', format='png')
        plt.close()
...
def write_statistics(all_statistics_of_all_runs, cwd):


    with open("stat_file.txt", "w") as f:
        for stats in all_statistics_of_all_runs:
            f.write('{0}\n'.format(stats))
...
if __name__ == '__main__':
    ...
    for instance_index, directory in enumerate(directories):
        ...
        write_statistics(sorted_result.stats, '')
        plotter.create_generation_plot(sorted_result.stats)
